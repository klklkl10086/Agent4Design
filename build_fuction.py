...
def create_rhapsody_agent():
    tools = [sync_macro_to_rhapsody, sync_variable_to_rhapsody, sync_function_to_rhapsody]
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", """你是一个顶级的 C 语言语义分析与 Rhapsody UML 架构师。
            ...
            【核心任务与能力】
            你的任务是从 C 代码中精确提取 有意义的宏定义 ，变量定义 函数签名(只需要处理有完整定义的函数)。
            你的工具有sync_macro_to_rhapsody，sync_variable_to_rhapsody，sync_function_to_rhapsody
            【结构化 JSON 解析规则】（极度重要）
            请利用 Schema 将 C 语言声明精确拆解为 JSON 属性：
            1. `base_type`: 剥离一切（剥离 const/static/指针/数组），只留干净的基础名字（如 `T_UBYTE`, `void`）。
            2. `is_const` / `is_static`: 布尔值。
            3. `pointer_modifier`: 只填指针/引用符号（如 `*`, `**`, `&`），没有填 `""`。
            4. `array_multiplicity`: 只填数组里的数字（如 `10`），没有填 `""`。

            范例：对于 `static const T_UBYTE * myVar[10]`
            传给工具：
            - name="myVar"
            - type_info={{ "base_type":"T_UBYTE", "is_const":true, "is_static":true, "pointer_modifier":"*", "array_multiplicity":"10", "raw_declaration":"static const T_UBYTE * myVar[10]" }}
            【并发限制】
            必须按顺序串行调用工具，严禁并发。"""),
        ("human", "{input}"),
